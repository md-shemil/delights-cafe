...
def admin_portal():
    ...
    cursor.execute('''
        SELECT orders.id, user_name, email_id, address, phone_number, payment_option, menu.name, orders.quantity,orders.total_amount
        FROM orders
...
def checkout():
    if request.method == 'POST':
        # Get user details and payment option from the form
        ...
        for menu_id, quantity in cart_items:
            cursor.execute('INSERT INTO orders (user_name, email_id, address, phone_number, payment_option, menu_id, quantity, total_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                           (user_name, email_id, address, phone_number, payment_option, menu_id, quantity, total_amount))
...
def show_bill():
    ...
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    cursor.execute('SELECT user_name, email_id, address, phone_number, payment_option, total_amount FROM orders ORDER BY id DESC LIMIT 1')
    order_details = cursor.fetchone()

    if order_details:
        user_name, email_id, address, phone_number, payment_option, total_amount = order_details
        ...
        return render_template('bill.html', user_name=user_name, email_id=email_id, address=address,
                               phone_number=phone_number, payment_option=payment_option,
